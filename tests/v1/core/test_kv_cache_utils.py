...
def test_get_kv_cache_config_one_worker():
    ...
    # is unified by padding the smaller physical page to the max page size.
    kv_cache_specs_hybrid = {
        "layer_1": new_kv_cache_spec(head_size=64),
    ...
    kv_cache_config_hybrid = get_kv_cache_configs(
        vllm_config, [kv_cache_specs_hybrid], [padded_page_size * 32]
    )[0]
    assert kv_cache_config_hybrid == KVCacheConfig(
        num_blocks=32,
        kv_cache_tensors=[
            KVCacheTensor(
                size=padded_page_size * 32, shared_by=["layer_1", "layer_2"]
            ),
        ],
        kv_cache_groups=[
            KVCacheGroupSpec(
