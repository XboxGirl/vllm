...
def _resolve_backend_kernels(
    ...
    logger.info_once(
        ...
        "%s layers; falling back to normal kernel selection for this layer.",
        linear_backend,
        linear_backend,
        layer_desc,
        scope="global",
...
_POSSIBLE_NVFP4_KERNELS: dict[PlatformEnum, list[type[NvFp4LinearKernel]]] = {
    PlatformEnum.CUDA: [
        FlashInferCuteDslNvFp4LinearKernel,
        FlashInferCutlassNvFp4LinearKernel,
        # Listed after CUTLASS so auto-selection remains unchanged; opt in
        # explicitly with --linear-backend flashinfer_b12x.
        FlashInferB12xNvFp4LinearKernel,
        CutlassNvFp4LinearKernel,
        MarlinNvFp4LinearKernel,
        FlashInferTrtllmNvFp4LinearKernel,
