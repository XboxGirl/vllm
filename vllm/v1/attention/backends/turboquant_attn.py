...
class TurboQuantMetadataBuilder(AttentionMetadataBuilder[TurboQuantMetadata]):
    ...
    # Context-aware downgrade. Keep UNIFORM_BATCH as the ClassVar default
    # (full caps for non-spec-decode setups), and override get_cudagraph_support
    # classmethod to downgrade to UNIFORM_SINGLE_TOKEN_DECODE only when
    # speculative_config is active.
    ...
    # → drafter outputs collapse to high-bias tokens (e.g. `
    ...
    def get_cudagraph_support(
        ...
        if os.environ.get("GENESIS_ENABLE_P65_TURBOQUANT_SPEC_CG_DOWNGRADE", "").lower() not in ("1", "true", "yes", "on"):
